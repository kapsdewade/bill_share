ActiveAdmin.register User, :namespace => :super_admin do
  ...
  member_action :activate, :method => :get do
    ...
    if @user.activate!
      redirect_to super_admin_users_path, :notice => "User activated successfully" and return
    end
  end
  ...
  member_action :deactivate, :method => :get do
    @user = User.find(params[:id])
    if @user.deactivate!
      redirect_to super_admin_users_path, :notice => "User deactivated successfully" and return
    end
  end
  ...
  batch_action :deactivate, :confirm => "Are you sure you want to deactivate? All subtopics related to this topic will set to inactive state" do |collection_selection|
    ...
    redirect_to super_admin_users_path, :notice =>"Users deactivated successfully."
  ...
  batch_action :activate do |collection_selection|
    ...
    redirect_to super_admin_users_path, :notice =>"Users activated successfully."
  ...
  index do
    ...
    column do |user|
      if user == current_user
        link_to("View", super_admin_user_path(user)) + ' | ' + link_to("Edit", edit_super_admin_user_path(user))
      else
        if user.inactive?
          link_to("View", super_admin_user_path(user)) + " | " + link_to("Edit", edit_super_admin_user_path(user))  + " | " + link_to("Activate", activate_super_admin_user_path(user))
        else
          link_to("View", super_admin_user_path(user)) + " | " + link_to("Edit", edit_super_admin_user_path(user))  + " | " + link_to("Deactivate", deactivate_super_admin_user_path(user))
        end
      end
    end
    # default_actions
  ...
  controller do
    ...
    def update
      update! do |format|
        ...
        redirect_to super_admin_users_path, :notice =>"User has been update successfully" and return
      end
    end
